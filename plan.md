...
### 任务 30：排序领域层
- **文件**：`src/domain/tasks.js`
- **描述**：新增 `normalizeSort(sort, order)` 解析并校验排序字段与方向；实现 `compareBySort(state, sort, order)`：
  - `priority`：数值比较，默认 `desc`（3→0）
  - `dueDate`：日期比较，**无到期日始终排最后**，默认 `asc`
  - `assignee`：按指派人用户名（`localeCompare`）比较，**公共任务（`assigneeId` 为 `null`）始终排最后**，默认 `asc`
  - `status`：按 `waiting → doing → done` 语义顺序，默认 `asc`
  - **兜底**：主字段同值时依次按 `priority` 降 → `dueDate` 升（无日期最后）→ `createdAt` 升 → `id` 升
- **验证**：新增 `test/tasks.sort.test.js`，单测覆盖四维度 × 升降序、默认方向、无到期日排最后、公共任务排最后、兜底顺序稳定
- **依赖**：任务 13
- **预计**：15 分钟

### 任务 31：排序路由参数
- **文件**：`src/routes/tasks.js`
- **描述**：`parseFilters` 增加 `sort`（缺省 `priority`）与 `order`（缺省按字段默认方向）；非法取值返回 `400 排序字段无效`
- **验证**：`test/api.tasks.test.js` 增加：`sort=priority` 默认降序、`sort=dueDate` 升序、非法 `sort` 返回 400
- **依赖**：任务 30
- **预计**：10 分钟

### 任务 32：前端排序交互
- **文件**：`public/index.html`、`public/css/style.css`、`public/js/tasks.js`、`public/js/filter.js`
- **描述**：
  - 主清单新增**排序 chip 行**：`优先级`（默认选中 `desc`）/ `指派人` / `到期日` / `状态`，chip 后缀显示 `↑`/`↓`
  - 点击已选维度切换升 ⇄ 降序；点击未选维度切到它并使用默认方向
  - 渲染逻辑：仅 `sort === 'dueDate'` 时输出日期分组标题，其余维度**平铺**
  - `FP.state.sort` / `FP.state.order` 纳入 `toApiParams()` 与 URL 同步（`readUrl` / `syncUrl`），刷新后可恢复
- **验证**：切换维度与方向后列表顺序符合预期；按到期日排序出现分组、其它维度无分组；URL 带 `sort`/`order` 且刷新后保持
- **依赖**：任务 31
- **预计**：20 分钟

### 任务 33：E2E 排序用例与回归
- **文件**：`e2e/tests/sorting.spec.js`
- **描述**：覆盖 TC-SORT-01 默认优先级降序平铺 / 02 切到到期日恢复分组 / 03 切换方向 / 04 按状态与指派人排序 / 05 URL 同步
- **验证**：`npx playwright test --config=e2e/playwright.config.js --workers=1` 全绿；`npm test` 全绿
- **依赖**：任务 32
- **预计**：15 分钟
...
**批次与检查点**：批次 1（任务 1–4）→ 检查点 1 → 批次 2（5–7）→ 检查点 2 → 批次 3（8–10）→ 检查点 3 → 批次 4（11–14）→ 检查点 4 → 批次 5（15–16）→ 检查点 5 → 批次 6（17–24）→ 检查点 6 → 批次 7（25–29）→ 检查点 7 → **批次 8（30–33）→ 检查点 8**。
